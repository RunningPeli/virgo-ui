...
export const useZIndex = createGlobalState((defaultBaseZIndex?: number, app?: App) => {
	if (!defaultBaseZIndex || !app) throw new Error('[Virgo] `useZIndex` composable must be initialized before usage.')
	...
	const baseZIndex = computed(() => {
		...
		return isNumber(injectedZIndexValue) ? injectedZIndexValue : defaultBaseZIndex
	})
